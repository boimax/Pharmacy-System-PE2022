const socket = io() //io("https://chat.tiemthuocgiadinh.click");
     ...
     {headers: {
      ...
      .then(data => {
        ...
        for (var i = 0, len = userCards.length; i < len; i++) {
            ...
            else if (e.target.classList.contains("avatar-conv") || e.target.classList.contains("client-info-left"))
            {
            ...
            {
                room = e.target.parentElement.parentElement.querySelector("[data-body]").firstChild.nodeValue.toString()
                username = e.target.parentElement.parentElement.querySelector("[data-header]").firstChild.nodeValue
                activeRoom = room
                activeUsername = username
                activeAvatar = e.target.parentElement.parentElement.querySelector(".avatar-conv").src
            }
            users.forEach(user => {
                user.element.classList.toggle("hide", true);
            })
